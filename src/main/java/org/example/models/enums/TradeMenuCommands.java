...
public enum TradeMenuCommands implements Command{
    ...
    TRADE_SYNTAX_ERROR("\\s*trade\\s+-u\\s(?<receiver>\\S+)\\s+-t\\s(?<tradeType>\\S+)\\s+-i\\s(?<item>\\S+)\\s+-a\\s(?<amount>\\S+)\\s+-p\\s(?<price>\\S+)\\s+-ti\\s(?<targetItem>\\S+)\\s+-ta\\s(?<targetAmount>\\S+)\\s*"),
    TRADE_WITH_GOODS("\\s*trade\\s+-u\\s(?<receiver>\\S+)\\s+-t\\s(?<tradeType>\\S+)\\s+-i\\s(?<item>\\S)\\s+-a\\s(?<amount>\\S+)\\s+-ti\\s(?<targetItem>\\S+)\\s+-ta\\s(?<targetAmount>\\S+)\\s*"),
